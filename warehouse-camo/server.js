...
(function() {
  var Crypto, Fs, Http, Https, Path, QueryString, Url, accepted_image_mime_types, bind, camo_hostname, content_length_exceeded, content_length_limit, content_length_limit_redirect, current_connections, debug_log, default_security_headers, error_log, finish, four_oh_four, hexdec, keep_alive, logging_enabled, max_redirects, port, process_url, server, shared_key, socket, socket_timeout, started_at, three_oh_three, total_connections, version,
  ...
  three_oh_three = function(resp, location) {
    resp.writeHead(303, {
      "Location": location
    });
    return finish(resp, "See Other");
  };
    ...
    if (url.host != null) {
      ...
      srcReq = Protocol.get(requestOptions, function(srcResp) {
        ...
        if (content_length > content_length_limit) {
          srcResp.destroy();
          return content_length_exceeded(resp, "Content-Length exceeded", url);
        } else {
          newHeaders = {
